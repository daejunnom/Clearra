...
사용자가 설명한 S23 Ultra의 **7은 논리 프로세서 수가 아니라, 전체 CPU 사용
옵션을 켜지 않은 계산 워커 수**였다. 이후 정책 해석은 8 logical / 7 compute로
정정한다. 아래 당시 실험의 reported 7 / compute 6 또는 7이라는 실제 입력은
역사적 수치로 보존하며 8/7 실험으로 바꿔 적지 않는다. 모바일 추가 계측은 중단했다.

이후의 순수 알고리즘 후보 C와 4195 직렬 batch count 결과는
`minimum-canonical-interval-ab-2026-09-08.md`에 분리했다. 그 비교는 실제 데스크톱
12 logical / 11 compute, 같은 M2 스케줄링에서 진행하며 미세 최적화를 포함하지 않는다.
