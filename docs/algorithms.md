...
- Core executor orchestration is the M17 product execution spine. `CoreExecutor` routes a compiled `SearchProblem` into `PcService`, `SetupService`, `CoverService`, or `PercentService`, then PC execution runs through `PackingRunner::run`, `BuildUpRunner::run`, accepted C `BuildVariant` views, C coverage row views, Rust `ObjectiveReducer::reduce`, and `CoreExecutionResult`. CLI remains outside the C ABI and only calls the executor facade. Coverage rows are derived from accepted BuildVariant buffers, not raw PackingCandidate rows, so rejected BuildUp candidates cannot contribute coverage.
