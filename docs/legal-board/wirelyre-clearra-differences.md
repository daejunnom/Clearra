...
> 2026-09-23 후속 상태: 아래 수치와 `R_k` 생성 방식 설명은 2026-09-22의
> 역사적 측정 기록이다. 현재 `legal-board-run`은 독립 `F_0…F_10` 생성 후
> 완성 필드에서 `F_k`로 제한한 exact ILC 역추적을 수행해 `L_k`를 만든다.
> 이 문서의 `현재는 R_k까지만 생성`이라는 문장은 당시의 상태를 뜻하며,
> 현재 구현·완료 여부의 권위는 상위 통합 계획과 실제 소스에 있다.
> 아직 다섯 profile의 최종 `L_k` 자산은 생성·자격 완료되지 않았다.
