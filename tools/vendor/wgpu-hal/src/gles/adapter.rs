...
impl super::Adapter {
    ...
    pub(super) unsafe fn expose(
        ...
        let (vendor_const, renderer_const) = if extensions.contains("WEBGL_debug_renderer_info") {
            ...
            #[cfg(all(target_os = "emscripten", feature = "gles"))]
            if unsafe {
            ...
            #[cfg(not(all(target_os = "emscripten", feature = "gles")))]
            (GL_UNMASKED_VENDOR_WEBGL, GL_UNMASKED_RENDERER_WEBGL)
        } else {
            (glow::VENDOR, glow::RENDERER)
        };
        ...
        let shading_language_version = {
            ...
            if full_ver.is_some() {
                ...
                naga::back::glsl::Version::Embedded {
                    version: value,
                    is_webgl: cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))),
                }
            }
        };
        ...
        downlevel_flags.set(
            ...
            !(cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))) || is_angle),
        );
        // see https://registry.khronos.org/webgl/specs/latest/2.0/#BUFFER_OBJECT_BINDING
        downlevel_flags.set(
            wgt::DownlevelFlags::UNRESTRICTED_INDEX_BUFFER,
            !cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))),
        );
        downlevel_flags.set(
        ...
        let bcn_exts = if cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))) {
            &webgl_bcn_exts[..]
        ...
        let has_etc = if cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))) {
        ...
        {
            #[cfg(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"))]
            {
                if context
                    .glow_context
            ...
            #[cfg(any(not(target_arch = "wasm32"), all(target_os = "emscripten", feature = "gles")))]
            {
                features.insert(wgt::Features::TEXTURE_COMPRESSION_ASTC);
                features.insert(wgt::Features::TEXTURE_COMPRESSION_ASTC_SLICED_3D);
                features.insert(wgt::Features::TEXTURE_COMPRESSION_ASTC_HDR);
        ...
        private_caps.set(
            ...
            cfg!(any(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "gles"), all(target_os = "emscripten", feature = "gles"))) || full_ver.is_some(),
        );
...
#[cfg(any(not(target_arch = "wasm32"), all(feature = "fragile-send-sync-non-atomic-wasm", not(target_feature = "atomics"))))]
