...
impl WasmDistributedCompletionSession {
    ...
    pub fn advance_parallel_warm_guarded(
        ...
        let result = self.execution.advance_minimum_parallel_warm(
    ...
    pub fn prepare_parallel_guarded(
        ...
        let shared_controller = self.parallel_host_grant.is_some_and(|(compute, _)| {
            self.maximum_compute_workers >= compute
        }) || (self.parallel_remote_memory_cap.is_some() && !self.parallel_control_only);
        if shared_controller && self.published_parallel_query
            != self.execution.minimum_parallel_query().map(|query| query.identity())
