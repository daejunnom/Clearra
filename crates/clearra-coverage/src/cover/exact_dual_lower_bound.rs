...
pub(super) fn checked_residual_dual_memory_projection(
    ...
    // patterns + row_counts + eligible_rows + source_to_eligible +
    ...
    // Candidate and best weights plus exact row loads.
    let certificate_slots = constraints.checked_mul(2)?.checked_add(rows)?;
