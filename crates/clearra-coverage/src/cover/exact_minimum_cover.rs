...
impl WitnessShortcutSession {
    ...
    fn checked_retained_capacity_bytes(&self) -> Result<u128, ExactMinimumCoverError> {
        ...
        bytes = match &self.phase {
            WitnessShortcutPhase::Breakout { search, .. } => bytes
    ...
    fn advance(
        ...
        while self.next_trial < slice_end {
            ...
            while !is_superset(&self.covered, target) {
                ...
                let choice = self.random_state as usize % restricted_candidate_count;
    ...
    fn advance_one(
        ...
        for (word_index, target_word) in target.iter().copied().enumerate() {
            ...
            while bits != 0 {
                ...
                let pattern = word_index * u64::BITS as usize + bit;
                bits &= bits - 1;
                if self.coverage_counts[pattern] != 0 {
                    continue;
                }
                if arbitrary_pivot {
        ...
        let pivot = self.pivot_candidates
            [next_breakout_random(&mut self.random_state) as usize % self.pivot_candidates.len()];
            ...
            .filter(|row_index| !self.selected[*row_index])
        {
            let make_gain = rows[add_row].words.iter().zip(target).enumerate().fold(
                ...
                |gain, (word_index, (row_word, target_word))| {
                    ...
                    while bits != 0 {
                        let bit = bits.trailing_zeros() as usize;
                        let pattern = word_index * u64::BITS as usize + bit;
                        added += usize::from(self.coverage_counts[pattern] == 0);
                        bits &= bits - 1;
                    }
                    ...
                    .fold(
                        ...
                        |loss, (word_index, ((remove_word, add_word), target_word))| {
                            ...
                            while bits != 0 {
                                let bit = bits.trailing_zeros() as usize;
                                let pattern = word_index * u64::BITS as usize + bit;
                                removed += usize::from(self.coverage_counts[pattern] == 1);
                                bits &= bits - 1;
                            }
            ...
            let remove_row = self.remove_candidates[next_breakout_random(&mut self.random_state)
                as usize
        ...
        let choice = next_breakout_random(&mut self.random_state) as usize % choice_count.max(1);
...
fn quotient_redundant_target_constraints_with_memory_guard(
    ...
    for row_index in 0..rows.len() {
        let current_rows_live = checked_dense_rows_retained_bytes(rows)?;
        ...
        memory_guard(
            checked_dense_rows_retained_bytes(rows)?
                .checked_add(target_live)
                .and_then(|bytes| bytes.checked_add(target_weights_live))
                .and_then(|bytes| bytes.checked_add(index_live))
                .and_then(|bytes| bytes.checked_add(compact_target_live))
...
impl ExactCoveredStateMemo {
    ...
    fn should_prune_or_record(
        ...
        let requested_growth = checked_requested_growth_bytes(&self.entries, 1)?;
        memory_guard(
            external_live_bytes
                .checked_add(memo_live)
                .and_then(|bytes| bytes.checked_add(state_live))
                .and_then(|bytes| bytes.checked_add(requested_growth))
                .ok_or(ExactMinimumCoverError::ProjectionOverflow)?,
        )?;
...
impl MinimumCoverSearch {
    ...
    fn prepare_reduced_node(
        ...
        for (row_index, row) in rows.iter().enumerate() {
            ...
            largest_gains[top_gain_slots - 1] = gain;
            largest_gains[..top_gain_slots].sort_unstable_by(|left, right| right.cmp(left));
        }
        if max_gain == 0 {
            #[cfg(feature = "diagnostic-probes")]
            self.record_top_gain_hot_cost(top_gain_started);
...
const WITNESS_ASSISTED_BREAKOUT_SWAP_BUDGET: usize = 1_000;
const WITNESS_ASSISTED_PREFERRED_TOTAL_BREAKOUT_SWAP_BUDGET: usize = 8_000;
    ...
    'trials: for trial_offset in 0..trial_budget {
        ...
        while !is_superset(&covered, target) {
            ...
            *random_state ^= *random_state << 17;
            ...
            let choice = *random_state as usize % restricted_candidate_count;
    ...
    'restarts: for restart in 0..restart_count {
        ...
        for iteration in 0..swaps_per_restart {
            ...
            let pivot = pivot_candidates
                [next_breakout_random(&mut random_state) as usize % pivot_candidates.len()];
                ...
                .filter(|row_index| !selected[*row_index])
            {
                ...
                let remove_row = remove_candidates
                    [next_breakout_random(&mut random_state) as usize % remove_candidates.len()];
            ...
            let choice = next_breakout_random(&mut random_state) as usize % choice_count.max(1);
