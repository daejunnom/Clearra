...
#[test]
fn complete_build_replay_height_is_bounded_before_execution() {
    let error = CliCommandParser::parse(
        "clearra build-probability --base-mask 0 --target-mask 0xf --height 7 --queue I --no-hold --no-mirror --result-mode complete-replay-paths",
    )
    .expect_err("height seven Build replay must fail closed");
    assert_eq!(error.code(), CliCommandErrorCode::InvalidValue);
}
