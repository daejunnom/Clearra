...
fn gui_minimum_solutions_finite_bag_is_the_only_supply_window_authority() {
    let request = CliCommandParser::parse(
        "clearra build cover --base-mask 0x0000000000000000 --target-mask 0x000000000000000f --height 4 --hold empty --patterns P2 --queue-knowledge oracle --objective min-cover --rule srs-plus --backend cpu --no-backend-fallback --workers 1",
    )
