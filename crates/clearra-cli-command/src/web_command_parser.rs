...
fn parse_build_probability_command(
    ...
    if matches!(
        ...
        BuildProbabilityResultMode::CompleteReplayPaths
            ...
            | BuildProbabilityResultMode::FixedQueueMaximumScore
            | BuildProbabilityResultMode::HighestScoreMinimumSet
    ) && base_mask.zip(target_mask).is_some_and(|(base, target)| {
        (base[0] | target[0]) >> 60 != 0
            || base[1..].iter().chain(&target[1..]).any(|word| *word != 0)
    }) {
        return Err(WebCommandError::new(
            WebCommandErrorCode::InvalidValue,
            "Build replay and score result modes currently require existing and target cells within the bottom six rows; empty display rows are allowed",
        ));
    }
