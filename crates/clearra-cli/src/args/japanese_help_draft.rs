...
テーブルベースの失敗時は、--tablebaseと--offline-fallbackを同時に明示した場合に限り、同じ正確なオフライン探索を開始します。キャンセル後に処理を再開することはありません。
...
指定した逆向き検索のPC目標に到達できないミノ順を、成功集合の正確な補集合として返します。--failed-countを省略すると失敗するミノ順をすべて具体化します。"#
        ...
        CliHelpTopic::Setup => {
            r#"使い方: clearra setup-finder --remaining IOTSZJL [--initial-hold empty|I|O|T|S|Z|J|L] [--mode oracle] [--queue-knowledge oracle|visible-7] [--next-cycle-remaining IOTS] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--workers N|--cpu-threads N|--auto-workers N] [--use-all-cpu-threads] [--tablebase [--offline-fallback]|--no-tablebase] [--allow-post-cycle-borrow]
  または: clearra setup-finder --remaining TI --mode qb --qb OS [--queue-knowledge oracle|visible-7] [--next-cycle-remaining OOSITZ] [--initial-hold empty|I|O|T|S|Z|J|L] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--workers N|--cpu-threads N|--auto-workers N] [--use-all-cpu-threads] [--tablebase [--offline-fallback]|--no-tablebase]
PC解法群の途中までの構築を探します。Sfinderのsetupにある必須領域への配置コマンドとは異なります。setupは従来の別名です。--remainingは次のバッグ境界までに残るミノの順序なし集合であり、ホールドを分離する前の集合からPC周期を決定します。2個含められるミノの種類は最大1種類です。重複するミノ1個は自動で初期ホールドに入りますが、PC周期の個数には含まれます。--initial-holdはCLI専用の明示指定で、同じ集合から該当ミノを1個取り除きます。--qbには次のバッグの固定プレフィックスまたはキューパターンを指定します。固定のOSは次バッグの先頭がO、次がSで、残り5個はランダムな順序であることを表します。[OS]!はOSとSOの両方を許可します。remainingの個数とQBシーケンス長の合計は最大11です。QB供給は、全未来を既知とするoracleのカバー率か、先読み7個に基づく厳密な行動方針かを選ぶ--queue-knowledgeとは独立しています。--next-cycle-remainingはPC後に残すホールドとバッグ内の残りミノを厳密に制限し、oracleとQBの両モードで使用できます。--tablebaseは明示的な機能であり、選択したプロファイルの4ラインSetupSearchが全解と差分検証を完了するまではfail-closedとなります。失敗時は--offline-fallbackを明示した場合に限り同じ正確なオフライン探索を開始し、キャンセル後には再開しません。--ruleはセットアップと完成形のすべてのBuildUp検証に使用するキック表を選びます。既定の検索並列数の上限は、プロセスから利用できる論理プロセッサ数から1を引いた値です。--auto-workersは固定並列実行を強制せず、この自動上限を下げます。--use-all-cpu-threadsを指定すると予約していた1個も使用します。--max-setup-piecesの既定値は9です。完成済みのPC解法を含める場合は10を指定します。優先度allは構築とPCの同時カバー率で順位を付けます。セットアップの長さは独立した設定です。autoはall/buildでは長い形、pcでは短い形を優先します。"#
        }
        CliHelpTopic::Cover => {
...
PC専用の幾何学的配置検索を実行し、供給されるミノと正確に適合する配置群を返します。BuildUp、到達可能性、カバー率、確率、ルール、スピン、B2B、スコア、visible-7、tablebase、依存DAG、実行制約の意味付けは利用できません。実際には組めない配置が含まれる場合があります。汎用形式の`clearra pc --tiling-only`と`clearra pc --objective tiling`は上級者向けの汎用PCリクエストとして維持され、この専用結果と同じ意味にはなりません。"#
        ...
        ProductHelpTopic::PcMinimals => {
            r#"使い方: clearra pc minimals --lines 2 [--patterns PATTERN | --queue IOTSZJL] [--hold|--no-hold] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--backend auto|cpu|gpu|hybrid] [--workers N|--auto-workers N] [--tablebase [--offline-fallback]|--no-tablebase] [--max-patterns N] [--max-nodes N] [--max-frontier-states N] [--max-candidates N]
...
最小解法集合の専用検索を実行します。入力全体のカバー状況を完全にリプレイ検証した後、その検索条件に対する正確な最小カバーを返します。明示的なメモリ上限、スコア、tiling-only、visible-7、依存DAGは利用できません。--tablebaseは正確に適格化された完全候補群だけを使用します。失敗時は--offline-fallbackを明示した場合に限り同じ正確なオフライン探索を開始し、キャンセル後には再開しません。トップレベルのminimalsとsfinder minimalsは従来互換の汎用結果を返します。"#
        }
        ProductHelpTopic::PcPath => {
            r#"使い方: clearra pc path --lines 2|4|6 (--queue QUEUE | --patterns PATTERN) [--hold|--no-hold] [--rule RULE] [--tablebase [--offline-fallback]|--no-tablebase] [検索オプション]
  または: clearra pc path --board-mask HEX --height 1..6 --pieces N --lines same-as-height (--queue QUEUE | --patterns PATTERN) [--hold empty|PIECE|--no-hold] [--rule RULE] [--tablebase [--offline-fallback]|--no-tablebase] [検索オプション]
objective all、count allで完全なリプレイ経路を専用検索します。各手順は配置、元のミノ列、ホールドと読み取り位置の遷移、消費ミノ数、ライン消去を保持します。--tablebaseは正確に適格化された完全候補群だけを使用します。失敗時は--offline-fallbackを明示した場合に限り同じ正確なオフライン探索を開始し、キャンセル後には再開しません。最適集合の同率候補ではなく、同率集合のメタデータやページ継続カーソルは含みません。"#
        }
...
PC確率の専用検索を実行し、入力ミノ順全体に対する完全な確率を返します。トップレベルのchanceとpercentは従来互換の汎用結果を返します。"#
  ...
  または: clearra pc score --board-mask HEX --height 1..6 --pieces N --lines same-as-height [--patterns PATTERN | --queue QUEUE] [スコアオプション] [CPUワーカーオプション] [--tablebase [--offline-fallback]|--no-tablebase]
ネイティブCPU実行には通常のローカルワーカー設定を使用します。自動実行は--use-all-cpu-threadsがない限り論理プロセッサを1個予約し、--workersは固定並列数を指定します。ブラウザーでは管理側がNを保持し、分離された各WASM子処理を1ワーカーに正規化してワーカープールの入れ子を防ぎます。--tablebaseは適格化された完全候補ソースだけを置換し、このスコアreducerを維持します。失敗時は--offline-fallbackを明示した場合に限り同じ正確なオフライン探索を開始し、キャンセル後には再開しません。入力は元のミノ16個以内と、因数分解されたパターン式1個に制限されます。P7P7P2は記号的に処理できます。PC盤面ごとの平均スコアを返します。スコアは基本的な近似値であり、プロファイルごとの厳密値ではありません。トップレベルのscoreとsfinder scoreは従来互換の汎用結果を返します。"#
        }
...
専用のjstris-ultraスコアプロファイルとt-spinsスピンプロファイルを使用し、固定ミノ順の最高スコアを検索します。ネイティブCPU実行は通常のローカルワーカー設定を使用します。自動実行は--use-all-cpu-threadsがない限り論理プロセッサを1個予約し、--workersは固定並列数を指定します。ブラウザーでは管理側がNを保持し、分離された各WASM子処理は1ワーカーを使用します。最高スコアの同率判定と順序には整数のスコアだけを使用します。攻撃力は参考値であり、同率の順位決定には使用しません。通常の結果に最適集合の同率メタデータはありません。--tiesを明示すると同じ最高スコアを持つすべての手順を通常の解法群として表示します。--tie-snapshotは使用できません。"#
  ...
  または: clearra pc score-minimals --board-mask HEX --height 1..6 --pieces N --lines same-as-height [--patterns PATTERN | --queue QUEUE] [スコアオプション] [CPUワーカーオプション] [--tablebase [--offline-fallback]|--no-tablebase] [--ties --tie-snapshot PATH]
スコアだけを基準とするB-optionの最高スコア最小集合検索を実行します。ネイティブCPU実行は通常のローカルワーカー設定を使用します。自動実行は--use-all-cpu-threadsがない限り論理プロセッサを1個予約し、--workersは固定並列数を指定します。ブラウザーでは管理側がNを保持し、分離された各WASM子処理は1ワーカーを使用します。--tablebaseは適格化された完全候補ソースだけを置換し、スコアだけのreducerを維持します。失敗時は--offline-fallbackを明示した場合に限り同じ正確なオフライン探索を開始し、キャンセル後には再開しません。スコアの同率判定、候補の適格性、並び順、集合の構成、決定的な選択に攻撃力は使わず、参考値としてのみ扱います。--tiesがなければ一定の規則で並べた最初の集合を表示します。--tiesを明示すると、再開可能な正確なスナップショットを作成します。同じ最小個数を持つすべての最適集合を`clearra continue --tie-snapshot PATH --tie-cursor TOKEN`でページ単位で取得できます。"#
        }
...
目標ドキュメントと入力済み解法ドキュメントは異なる型として扱い、相互に代用できません。すべての形式で--queueと--patternsのいずれか一方が必須です。--queue-knowledge oracle|visible-7、--hold PIECE|--no-hold、--rule RULE、--max-patterns N、--max-nodes N、--max-frontier-states N、--max-candidates N、--workers N|--auto-workers N|--use-all-cpu-threads、--cpu-warmupを使用できます。--objective all|unique|min-cover|max-probability-minimum|max-score-coverのうち、各形式で許可された目的だけを指定できます。互換の別名はminimum-coverのみです。スコア形式だけが--score-profile tetrio|guideline|jstris-ultraと--initial-b2b 0..65535を受け付けます。正確な最適集合を返す形式（cover、congruent-cover、setup-cover、setup-cover-score、evaluate minimals、evaluate score）で別の同率解を取得するには、--ties --tie-snapshot PATHの明示が必要です。通常の解法群や確率結果では使用しません。スコアの同率判定と順序に攻撃力は使用しません。型付きBuildはv0.8ではCPU専用です。有限な応答の保証が実装されるまで--max-memory-mibは拒否されます。"#
        ...
        ProductHelpTopic::SetupScore => {
            r#"使い方: clearra setup score --document-format ctk3|fumen --document DOCUMENT (--setup-queue QUEUE | --setup-patterns PATTERN) (--solution-queue QUEUE | --solution-patterns PATTERN) [--clear 1..6] [--hold|--no-hold] [--score-profile tetrio|guideline|jstris-ultra] [--initial-b2b N] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--max-patterns N] [--workers N|--auto-workers N|--use-all-cpu-threads] [--legal-board|--no-legal-board] [--conditioned-reachability|--no-conditioned-reachability]。2つの正確な加速機能は、カバー率の計算とスコア計算の両方に同じ設定を使用します。適格なアセットがない場合や無効な場合は、従来の正確な検索を続行します。CPU専用で、GPUとバックエンドのフォールバックは使用できません。"#
        }
