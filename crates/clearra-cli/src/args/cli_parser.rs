    ...
    pub fn into_output(self, language: LanguageId) -> CliOutput {
        ...
        CliOutput::success(format!(
            ...
            match self {
                ...
                Self::Pc => {
                    "usage: clearra pc --lines 2 [--queue IOTSZJL] [--fixed|--observed] [--hold|--no-hold] [--queue-knowledge oracle|visible-7] [--objective all|unique|min-cover] [--solution-probabilities] [--score] [--score-profile tetrio|guideline|jstris-ultra] [--spin-profile t-spins|t-spins-plus|all-spin|all-spin-plus|all-mini|all-mini-plus] [--initial-b2b N] [--rule srs-plus|srs|srs-x|jstris-180|asc|ars|no-kick] [--kick-profile-json JSON] [--backend auto|cpu|gpu|hybrid] [--workers N|--cpu-threads N] [--use-all-cpu-threads] [--cpu-warmup] [--gpu-warmup] [--tablebase|--no-tablebase] [--build-dependency-dag|--no-build-dependency-dag] [--no-gpu] [--deterministic] [--max-candidates N] [--max-patterns N] [--max-memory-mib N] [--gpu-device auto|N] [--allow-backend-fallback|--no-backend-fallback]\ncompiles an opening preset into a SearchProblem; --tablebase is opt-in and uses a precomputed exact-dead PC4 index only on compatible WASM CPU geometry states, with the standard exact search retained for every unsupported or unknown state; --build-dependency-dag is opt-in and reuses the complete canonical-operation subset DAG, preserving every multi-parent convergence while skipping transitions that cannot reach completion; allocation failure or a stale graph keeps the baseline BuildUp search; --queue-knowledge oracle keeps the full-future upper bound while visible-7 requires one shared action for queues with the same currently visible seven pieces; --solution-probabilities computes each normalized solution's pattern-union probability without reordering CLI output; --score-profile selects scoring independently from the kick rule and spin profile; guideline is fixed at level 1"
                }
                Self::PcScenario => {
                ...
                Self::Setup => {
                    "usage: clearra setup --remaining IOTSZJL [--initial-hold empty|I|O|T|S|Z|J|L] [--mode oracle] [--queue-knowledge oracle|visible-7] [--next-cycle-remaining IOTS] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--tablebase|--no-tablebase] [--allow-post-cycle-borrow]\n   or: clearra setup --remaining TI --mode qb --qb OS [--queue-knowledge oracle|visible-7] [--next-cycle-remaining OOSITZ] [--initial-hold empty|I|O|T|S|Z|J|L] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--tablebase|--no-tablebase]\n--remaining is the unordered inventory before the next bag boundary, including the selected initial-hold piece. --initial-hold removes one matching piece from that inventory and starts it in hold; empty is the default. Without --initial-hold, each remaining piece kind must be unique. --qb is the observed next-bag piece group and enables queue-based setup generation; it is independent from --queue-knowledge, which selects full-future oracle coverage or an exact visible-seven action policy. --next-cycle-remaining independently constrains the exact hold plus bag remainder left after this PC and is valid in oracle or QB mode. --tablebase is opt-in and only rejects precomputed exact-dead PC4 completion states; all other states use the standard exact search. --rule selects the kick table used for every setup and completion BuildUp check. --max-setup-pieces defaults to 9; use 10 to include complete PC solutions. Priority all ranks joint Build x PC coverage. Setup length is independent; auto favors longer setups for all/build and shorter setups for pc"
                }
                Self::Cover => {
