    ...
    fn help_body(self) -> &'static str {
        match self {
            Self::BoundaryRecovery => {
                "usage: clearra recovery boundary --initial-board-mask HEX --target-board-mask HEX --height 1..25 --queue REFERENCE --stage-one-count N --placements N [--role-mask POSITION:HEX ...] [--queue-pattern PATTERN --max-pattern-evaluations N --max-total-states N] [--max-early-placements 0|1] [--borrow-source-position N --borrow-placement-mask HEX] [--hold|--no-hold] [--rule RULE] [--spin-profile PROFILE] [--initial-b2b 0|1] [--preserve-b2b-bag N ...] [--preserve-b2b-stage-one] [--preserve-b2b-stage-two] [--max-states 1..1000000]\nThe queue is a continuous 2..42-piece source. Repeat --role-mask for every required source position to require exact four-cell lock-time roles. With one early placement, select a second-stage source token; its role mask supplies the borrow placement, or give --borrow-placement-mask when roles are omitted. --preserve-b2b-bag independently enables B2B preservation for a one-based bag position, counting bags within each stage; partial bags restart at the stage boundary. The whole-stage flags remain accepted for older commands. Optional --queue-pattern computes weighted normal, additional recovery, and unresolved probabilities over complete seven-piece bags. It requires every reference bag and stage to be complete, all placement roles specified, and at most 100000 pattern identities. Pattern permutations bind roles and the selected early placement by bag and piece kind. Both modes use full future queue knowledge, preserve supply/hold/B2B continuity, and report incomplete separately from no path. Neither mode proves see-n playability."
            }
            Self::PcTiling => {
