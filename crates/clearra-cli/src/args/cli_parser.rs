    ...
    pub fn into_output(self, language: LanguageId) -> CliOutput {
        ...
        CliOutput::success(format!(
            ...
            match self {
                ...
                Self::Setup => {
                    "usage: clearra setup --remaining IOTSZJL [--initial-hold empty|I|O|T|S|Z|J|L] [--mode oracle] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--allow-post-cycle-borrow]\n   or: clearra setup --remaining TI --mode qb --qb OOSITZ [--initial-hold empty|I|O|T|S|Z|J|L] [--rule srs-plus|srs|srs-x|jstris-180] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10]\n--remaining is the unordered inventory before the next bag boundary, including the selected initial-hold piece. --initial-hold removes one matching piece from that inventory and starts it in hold; empty is the default. Without --initial-hold, each remaining piece kind must be unique. --qb is the exact next-cycle remaining inventory after this PC, including hold. --rule selects the kick table used for every setup and completion BuildUp check. --max-setup-pieces defaults to 9; use 10 to include complete PC solutions. Priority all ranks joint Build x PC coverage. Setup length is independent; auto favors longer setups for all/build and shorter setups for pc"
                }
                Self::Cover => {
