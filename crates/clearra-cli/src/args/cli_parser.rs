    ...
    fn help_body(self) -> &'static str {
        match self {
            Self::BoundaryRecovery => {
                "usage: clearra recovery boundary --initial-board-mask HEX --target-board-mask HEX --height 1..25 --queue FIXED --stage-one-count N --placements N [--role-mask POSITION:HEX ...] [--max-early-placements 0|1] [--borrow-source-position N --borrow-placement-mask HEX] [--hold|--no-hold] [--rule RULE] [--spin-profile PROFILE] [--initial-b2b 0|1] [--preserve-b2b-stage-one] [--preserve-b2b-stage-two] [--max-states 1..1000000]\nThe queue is one continuous, fully known source. Repeat --role-mask for every required one-based source position to require an exact four-cell lock-time placement. With one early placement, select a second-stage source token; its role mask supplies the borrow placement, or give --borrow-placement-mask when roles are omitted. The search proves a normal PC connection before an additional recovery, keeps hold, source identity, line clears, and B2B state continuous, and reports incomplete separately from no path. The final target checks occupied cells; lock-time roles are not post-clear colors. This command does not compute pattern probabilities or see-n playability."
            }
            Self::PcTiling => {
