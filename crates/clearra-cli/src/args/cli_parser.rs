    ...
    pub fn into_output(self, language: LanguageId) -> CliOutput {
        ...
        CliOutput::success(format!(
            ...
            match self {
                ...
                Self::FailedQueue => {
                    "usage: clearra failed-queue --lines 4 [--patterns P7P3 | --queue IOTSZJL --fixed|--observed] [--hold|--no-hold] [--queue-knowledge oracle|visible-7] [--rule srs-plus|srs|srs-x|jstris-180] [--backend auto|cpu|gpu|hybrid] [--workers N|--cpu-threads N|--auto-workers N] [--tablebase [--offline-fallback]|--no-tablebase] [--build-dependency-dag|--no-build-dependency-dag] [--failed-count N]\nreturns the exact complement of queues that reach the requested reverse-search PC target; omitted --failed-count materializes every failed queue. A tablebase failure starts the same exact offline request only when --offline-fallback is explicit; cancellation never restarts work"
                }
                Self::Setup => {
                    "usage: clearra setup-finder --remaining IOTSZJL [--initial-hold empty|I|O|T|S|Z|J|L] [--mode oracle] [--queue-knowledge oracle|visible-7] [--next-cycle-remaining IOTS] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--workers N|--cpu-threads N|--auto-workers N] [--use-all-cpu-threads] [--tablebase [--offline-fallback]|--no-tablebase] [--legal-board|--no-legal-board] [--conditioned-reachability|--no-conditioned-reachability] [--allow-post-cycle-borrow]\n   or: clearra setup-finder --remaining TI --mode qb --qb OS [--queue-knowledge oracle|visible-7] [--next-cycle-remaining OOSITZ] [--initial-hold empty|I|O|T|S|Z|J|L] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--priority all|build|pc] [--setup-length auto|longer|shorter] [--max-setup-pieces 1..10] [--workers N|--cpu-threads N|--auto-workers N] [--use-all-cpu-threads] [--tablebase [--offline-fallback]|--no-tablebase] [--legal-board|--no-legal-board] [--conditioned-reachability|--no-conditioned-reachability]\nthis PC-family partial-BuildUp finder is not Sfinder setup's required-area placement command; 'setup' remains a legacy alias. --remaining is the unordered inventory before the next bag boundary and determines the PC cycle before any hold separation. At most one piece kind may appear twice; that duplicated piece is automatically placed in initial hold and still counts toward the cycle. --initial-hold remains a CLI-only explicit override and removes one matching piece from the same inventory. --qb accepts an exact next-bag prefix or queue pattern and enables queue-based setup generation. Exact OS means O first and S second, with the other five bag pieces randomly ordered; [OS]! permits OS or SO. Remaining count plus QB sequence length may be at most 11. QB supply is independent from --queue-knowledge, which selects full-future oracle coverage or an exact visible-seven action policy. --next-cycle-remaining independently constrains the exact hold plus bag remainder left after this PC and is valid in oracle or QB mode. --tablebase is opt-in and fails closed until the selected profile's 4-line SetupSearch target has full-solution and differential qualification. Failure starts the standard offline exact search only when --offline-fallback is explicitly present; cancellation never restarts work. Qualified legal-board and conditioned-reachability assets are enabled by default, remain independent, and fail open to the existing exact path when absent, invalid, or out of scope. --rule selects the kick table used for every setup and completion BuildUp check. Setup search defaults to the process-visible logical processor count minus one; --auto-workers lowers that adaptive ceiling without forcing fixed parallel execution; --use-all-cpu-threads explicitly removes the reserved processor. --max-setup-pieces defaults to 9; use 10 to include complete PC solutions. Priority all ranks joint Build x PC coverage. Setup length is independent; auto favors longer setups for all/build and shorter setups for pc"
                }
                Self::Cover => {
...
impl ProductHelpTopic {
    fn help_body(self) -> &'static str {
        match self {
            ...
            Self::PcMinimals => {
                "usage: clearra pc minimals --lines 2 [--patterns PATTERN | --queue IOTSZJL] [--hold|--no-hold] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--backend auto|cpu|gpu|hybrid] [--workers N|--auto-workers N] [--tablebase [--offline-fallback]|--no-tablebase] [--max-patterns N] [--max-nodes N] [--max-frontier-states N] [--max-candidates N]\n   or: clearra pc minimals --board-mask HEX --height 1..6 --pieces N --lines same-as-height [--patterns PATTERN | --queue QUEUE] [--hold empty|PIECE|--no-hold] [search options]\nRuns the dedicated minimum-solution search and returns the exact query-bound minimum cover after complete source-coverage replay. Explicit memory caps, scoring, tiling-only, visible-7, and dependency-DAG semantics are unavailable. --tablebase uses only an exact qualified candidate family. Failure starts the same exact offline request only when --offline-fallback is explicit; cancellation never restarts work. The top-level minimals and sfinder minimals commands remain legacy-compatible generic results."
            }
            Self::PcPath => {
                "usage: clearra pc path --lines 2|4|6 (--queue QUEUE | --patterns PATTERN) [--hold|--no-hold] [--rule RULE] [--tablebase [--offline-fallback]|--no-tablebase] [search options]\n   or: clearra pc path --board-mask HEX --height 1..6 --pieces N --lines same-as-height (--queue QUEUE | --patterns PATTERN) [--hold empty|PIECE|--no-hold] [--rule RULE] [--tablebase [--offline-fallback]|--no-tablebase] [search options]\nRuns the dedicated complete replay-path search with objective all and count all. Every witness preserves its placements, source sequence, hold/cursor transitions, consumed-piece count, and line clears. --tablebase uses only an exact qualified candidate family. Failure starts the same exact offline request only when --offline-fallback is explicit; cancellation never restarts work. This is not an optimal-portfolio tie set and exposes no tie metadata or portfolio cursor."
            }
            Self::PcChance => {
