    ...
    fn help_body(self) -> &'static str {
        match self {
            Self::BoundaryRecovery => {
                "usage: clearra recovery boundary --initial-board-mask HEX --target-board-mask HEX --height 1..25 --queue FIXED --stage-one-count N --placements N [--max-early-placements 0|1] [--borrow-source-position N --borrow-placement-mask HEX] [--hold|--no-hold] [--rule RULE] [--spin-profile PROFILE] [--initial-b2b 0|1] [--preserve-b2b-stage-one] [--preserve-b2b-stage-two] [--max-states 1..1000000]\nThe queue is one continuous, fully known source. With one early placement, select a second-stage source token by one-based queue position and its exact four-cell lock-time mask. The search proves a normal PC connection before an additional recovery, keeps hold, source identity, line clears, and B2B state continuous, and reports incomplete separately from no path. The final target checks occupied cells, not all colored placement roles. This command does not compute pattern probabilities or see-n playability."
            }
            Self::PcTiling => {
