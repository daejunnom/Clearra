...
impl ProductHelpTopic {
    fn help_body(self) -> &'static str {
        match self {
            Self::BoundaryRecovery => {
                "usage: clearra recovery boundary --initial-board-mask HEX --target-board-mask HEX --height 1..25 --queue REFERENCE --stage-one-count N [--placements N|auto] [--stage-one-board-mask HEX] [--role-mask POSITION:HEX ...] [--queue-pattern PATTERN --max-pattern-evaluations N --max-total-states N|unlimited] [--max-early-placements 0|1] [--borrow-role-position N --borrow-placement-mask HEX] [--hold|--no-hold] [--rule RULE] [--spin-profile PROFILE] [--initial-b2b 0|1] [--preserve-b2b-bag N ...] [--preserve-b2b-stage-one] [--preserve-b2b-stage-two] [--max-states N|unlimited]
The queue is a continuous 2..42-piece source. Omitted --placements (or auto) checks every cell-conservation-compatible prefix; unused tokens remain lookahead. Exact role masks determine their own count. Normal connection is proven across all candidate counts before classifying any additional recovery. Omitted state limits (or unlimited) have no algorithmic state cutoff; cancellation and host resource protection remain active. The stage-one target is remaining initial/stage-one provenance after actual clears, excluding borrowed stage-two cells; omission keeps an empty checkpoint. Repeat --role-mask for every required placement role to require exact four-cell lock-time geometry. With one early placement, select a second-stage role; its mask supplies the borrow placement, or give --borrow-placement-mask when roles are omitted. --preserve-b2b-bag independently enables B2B preservation for a one-based supply bag position, counting bags within each stage; partial bags restart at the stage boundary. The whole-stage flags remain accepted for older commands. Optional --queue-pattern computes weighted normal, additional recovery, and unresolved probabilities over complete seven-piece bags. It requires every reference bag and stage to be complete, all placement roles specified, and at most 100000 pattern identities. Pattern permutations keep roles fixed while reordering supply tokens by bag. Both modes use full future queue knowledge, preserve supply/hold/B2B continuity, and report incomplete separately from no path. Neither mode proves see-n playability."
            }
            Self::PcTiling => {
            ...
            Self::PcMinimals => {
                "usage: clearra pc minimals --lines 2 [--patterns PATTERN | --queue IOTSZJL] [--hold|--no-hold] [--rule srs-plus|srs|srs-x|jstris-180|no-kick] [--backend auto|cpu|gpu|hybrid] [--workers N|--auto-workers N] [--tablebase|--no-tablebase] [--max-patterns N] [--max-nodes N] [--max-frontier-states N] [--max-candidates N]\n   or: clearra pc minimals --board-mask HEX --height 1..6 --pieces N --lines same-as-height [--patterns PATTERN | --queue QUEUE] [--hold empty|PIECE|--no-hold] [search options]\n   or: clearra pc pinned-minimals [the same PC source options] --required-format ctk3|fumen --required-document DOCUMENT [--expected-source-set-hash cts1:HEX]\nRuns the dedicated minimum-solution search and returns the exact query-bound minimum cover after complete source-coverage replay. The pinned command selects colored drawings from a prior complete all-solutions result, regenerates the full source and fails if a drawing is absent or ambiguous. Its optional source-set hash rejects stale selections. Explicit memory caps, scoring, tiling-only, visible-7, and dependency-DAG semantics are unavailable. --pin-key KEY remains a legacy advanced minimals input. --tablebase uses only an exact qualified candidate family and never starts offline fallback automatically. The top-level minimals and sfinder minimals commands remain legacy-compatible generic results."
            }
            Self::PcPath => {
                "usage: clearra pc path --lines 2|4|6 (--queue QUEUE | --patterns PATTERN) [--hold|--no-hold] [--rule RULE] [--tablebase|--no-tablebase] [search options]\n   or: clearra pc path --board-mask HEX --height 1..6 --pieces N --lines same-as-height (--queue QUEUE | --patterns PATTERN) [--hold empty|PIECE|--no-hold] [--rule RULE] [--tablebase|--no-tablebase] [search options]\nRuns the dedicated complete replay-path search with objective all and count all. Every witness preserves its placements, source sequence, hold/cursor transitions, consumed-piece count, and line clears. --tablebase uses only an exact qualified candidate family and never starts offline fallback automatically. This is not an optimal-portfolio tie set and exposes no tie metadata or portfolio cursor."
            }
            Self::PcChance => {
