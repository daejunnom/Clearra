    ...
    if help {
        let (note, unavailable, location) = match language {
            LanguageId::Ko => ("선택한 킥테이블의 그래프와 두 인덱스를 명시적으로 다운로드합니다. curl이 필요합니다. check는 용량만 확인하며 파일을 받지 않습니다. download는 최신 데이터로 갱신하고 이전 데이터는 성공 후 정리합니다. Ctrl+C로 중단할 수 있습니다.",
                ...
                "CLEARRA_PC4_DIRECTORY로 다운로드와 로컬 탐색의 공통 저장 위치를 지정합니다. 다운로드는 선택 사항입니다. --tablebase 탐색은 저장된 데이터가 없으면 필요한 구간만 온라인으로 조회합니다(curl 7.84 이상). 조회 실패 시 --offline-fallback을 함께 명시한 요청만 같은 정확 오프라인 탐색을 시작하며, 취소된 요청은 다시 시작하지 않습니다."),
            LanguageId::Ja => ("選択したキックテーブルのグラフと2つのインデックスを明示的にダウンロードします。curlが必要です。checkは容量のみ確認します。downloadは最新データへ更新し、成功後に旧データを削除します。Ctrl+Cで中止できます。",
                ...
                "CLEARRA_PC4_DIRECTORYで共通の保存先を指定します。ダウンロードは任意です。--tablebase検索は保存済みデータがなければ必要な範囲だけオンラインで取得します(curl 7.84以降)。失敗時は--offline-fallbackを併記した要求だけが同じ正確なオフライン検索を開始し、キャンセル後には再開しません。"),
            _ => ("Explicitly download the selected kick table's graph and two indexes. Requires curl. check retrieves sizes only. download updates to the latest data and removes the old copy only after success. Ctrl+C cancels.",
                ...
                "CLEARRA_PC4_DIRECTORY selects the shared storage base. Downloads are optional: --tablebase searches read online ranges when no data is installed (curl 7.84 or later). On lookup failure, only a request that also supplies --offline-fallback starts the same exact offline search; cancellation never restarts work.")
        };
