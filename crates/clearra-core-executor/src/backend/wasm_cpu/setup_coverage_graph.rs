//! SRP rationale: this module has one behavior-level change reason: canonicalizing exact setup
//! evidence into the compact coverage graph while preserving its storage, interning, and wire
//! invariants.
...
impl SetupCoverageEdge {
    ...
    pub(super) const fn max_child_reference() -> u32 {
        Self::CHILD_MASK
    }
...
impl SetupCoverageGraph {
    ...
    pub(super) fn edges_for_node(&self, node: SetupCoverageNode) -> &[SetupCoverageEdge] {
        let start = node.edge_start as usize;
