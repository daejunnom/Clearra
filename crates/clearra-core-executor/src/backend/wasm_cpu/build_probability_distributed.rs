...
const fn distributed_build_local_compute_units(
    replica_count: usize,
    separate_worker_instances: bool,
) -> usize {
    if separate_worker_instances {
        1
    } else {
        replica_count
    }
...
impl WasmBuildProbabilityCandidateProducer {
    ...
    pub fn new_with_finesse_and_verifiers_typed(
        ...
        // A browser coordinator executes in one single-threaded WASM instance;
        // the host owns the complete worker pool and each verifier admits its
        // own local compute slot. Charging every remote replica to this local
        // one-slot authority rejects all distributed Build searches. Native
        // in-process verifiers still borrow their slots from the parent lease.
        // Replica memory remains accounted for by the aggregate plan below.
        let local_compute_units =
            distributed_build_local_compute_units(replica_count, cfg!(target_family = "wasm"));
        let execution_admission = admit_budget_bound_search_execution(problem, local_compute_units)
...
mod tests {
    ...
    #[test]
    fn distributed_build_compute_admission_tracks_local_instances_not_remote_replicas() {
        use clearra_core_domain::resource::{
            ResourceLeaseCapacity, ResourceLeaseOwnerId, ResourceLeaseRequest,
            SharedResourceLeaseAuthority,
        };

        let wasm_capacity = ResourceLeaseCapacity::new(1, 0).expect("single WASM instance");
        let authority = SharedResourceLeaseAuthority::new(wasm_capacity);
        for replica_count in [1, 2, 11] {
            let local = distributed_build_local_compute_units(replica_count, true);
            assert_eq!(
                local, 1,
                "remote verifiers own separate WASM compute leases"
            );
            let _lease = authority
                .try_acquire(
                    ResourceLeaseOwnerId::new(replica_count as u64).expect("request owner"),
                    ResourceLeaseRequest::new(local as u32, 0).expect("local compute request"),
                )
                .expect("remote replica count must not exhaust local compute");
            assert_eq!(
                distributed_build_local_compute_units(replica_count, false),
                replica_count,
                "native delegated verifiers retain parent compute coverage"
            );
        }
    }
