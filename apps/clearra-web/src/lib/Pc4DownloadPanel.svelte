<script lang="ts">
  ...
  const texts = {
    ko: { title: 'TB 전체 다운로드', help: '선택한 킥테이블의 그래프와 두 인덱스만 저장합니다. 완성·호환성 확인이 된 프로필만 다운로드할 수 있습니다. 저장 후에는 필요한 구간을 로컬로 읽으며 다른 브라우저·주소와 공유되지 않습니다.',
      ...
      unsupported: '이 브라우저에서는 로컬 TB 저장을 지원하지 않습니다. 부분 조회는 계속 사용할 수 있습니다.', unavailable: '선택한 킥테이블은 아직 전용 인덱스와 호환성 확인이 완료되지 않아 다운로드할 수 없습니다.',
    ...
    en: { title: 'Download full TB', help: 'Store the selected kick table’s graph and two indexes. Only completed, compatible profiles can be downloaded. Searches read local slices. Storage is specific to this browser and site address.',
      ...
      unsupported: 'Local TB storage is unavailable in this browser. Partial online reads remain available.', unavailable: 'The selected kick table cannot be downloaded yet: its own indexes and compatibility have not been qualified.',
    ...
    ja: { title: 'TBを全体ダウンロード', help: '選択したキックテーブルのグラフと2つのインデックスのみ保存します。完成と互換性を確認できたプロファイルのみダウンロードできます。必要な部分をローカルで読み取り、別のブラウザやサイトとは共有されません。',
      ...
      unsupported: 'このブラウザではローカルTBを保存できません。オンラインの部分読み取りは引き続き利用できます。', unavailable: '選択したキックテーブルは専用インデックスと互換性の確認が完了していないため、まだダウンロードできません。',
...
<details class="pc4-download" on:toggle={(event) => { if ((event.currentTarget as HTMLDetailsElement).open) initialize(); }}>
  ...
  {:else}
    ...
    <div class="actions">
      <button type="button" disabled={busy || profile !== 'jstris-180'} on:click={() => { plan = null; act('prepare'); }}>{t.check}</button>
