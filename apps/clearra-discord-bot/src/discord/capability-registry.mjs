// This module is the Discord runtime authority for product capabilities. Keep
// it independent from the JSON contract fixture: the contract tests compare
// both projections field by field so either side can detect drift in the
// other.
...
const BUILD_V2_DISCORD_SURFACE_ROWS = Object.freeze(new Map([
  ["build.cover", {
    ...
    loweringAuthority: "discord.typed-command-lowering.v1",
...
const CAPABILITY_PRESENTATION = Object.freeze({
  "pc.path": ["Find every represented perfect-clear path", "Discord publishes only the smallest canonical candidate witness and exposes no tie or paging surface.", "path", "pc-path-v2"],
  ...
  "pc.best-save": ["Select clearra-save-v1 best-save witnesses", "Discord fixes exact witness ties to the smallest canonical candidate ID; typed results retain a normal winner list.", "best-save"],
  ...
  "pc.score-finder": ["Find the highest Jstris-score perfect clear for one exact queue", "Score equality and ordering are score-only. Discord selects the smallest canonical candidate ID and never uses informational attack as a selector.", "score-finder", "pc-score-finder-v2"],
...
export function assertDiscordGenericCompatibilityRoutes() {
  ...
  for (const route of discordGenericCompatibilityRoutes) {
    ...
    if (
      ...
      route.loweringAuthority !== "discord.generic-compatibility-lowering.v1"
    ) {
...
function genericCompatibilityRoute(row) {
  return {
    ...
    telemetryIdentity: row.id,
    loweringAuthority: "discord.generic-compatibility-lowering.v1",
    removeIn: "v0.10.0",
    lifetime: "long-term",
