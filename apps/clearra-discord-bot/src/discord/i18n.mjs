...
const CATALOGS = Object.freeze({
  en: Object.freeze({
    ...
    "result.kind.public.boundary_recovery": "boundary-recovery search",
    "recovery.result.status": "Outcome",
    "recovery.result.status.normal": "Normal PC connection",
    "recovery.result.status.pc-preserving-recovery": "PC-preserving boundary recovery",
    "recovery.result.status.non-pc-recovery": "Boundary recovery without PC",
    "recovery.result.status.no-path-within-declared-scope": "No path within the declared scope",
    "recovery.result.status.incomplete": "Search incomplete; no impossibility claim",
    "recovery.result.status.population-complete": "Weighted pattern search complete",
    "recovery.result.status.population-incomplete": "Weighted pattern search incomplete",
    "recovery.result.full_queue_knowledge": "Assumes the full future queue is known.",
    "recovery.result.states": "Normal + recovery states",
    "recovery.result.borrowed": "Early stage-two placements",
    "recovery.result.checkpoint": "Stage-one cleanup step",
    "recovery.result.evaluated": "Evaluated / possible patterns",
    "recovery.result.normal_probability": "Normal PC probability",
    "recovery.result.pc_recovery_probability": "PC-preserving recovery probability",
    "recovery.result.non_pc_recovery_probability": "Non-PC recovery probability",
    "recovery.result.additional_probability": "Additional recovery probability",
    "recovery.result.response_probability": "Total response probability",
    "recovery.result.no_path_probability": "No-path probability",
    "recovery.result.unknown_probability": "Unresolved probability",
    ...
    "input.options.setup_qb_required": "setup mode=qb requires a qb queue or pattern.",
    "input.options.setup_qb_oracle_conflict": "qb supply cannot be used with setup mode=oracle.",
    "input.options.setup_qb_bag_capacity": "the setup remaining count plus qb sequence length may be at most 11.",
  ...
  ko: Object.freeze({
    ...
    "result.kind.public.boundary_recovery": "경계 리커버리 탐색",
    "recovery.result.status": "결과",
    "recovery.result.status.normal": "정상 PC 연결",
    "recovery.result.status.pc-preserving-recovery": "PC 보존 경계 리커버리",
    "recovery.result.status.non-pc-recovery": "PC 없는 경계 리커버리",
    "recovery.result.status.no-path-within-declared-scope": "지정 범위에서 경로 없음",
    "recovery.result.status.incomplete": "탐색 미완료 · 불가능 판정 아님",
    "recovery.result.status.population-complete": "가중 패턴 탐색 완료",
    "recovery.result.status.population-incomplete": "가중 패턴 탐색 미완료",
    "recovery.result.full_queue_knowledge": "미래 공급 전체를 알고 선택한 결과입니다.",
    "recovery.result.states": "정상 + 리커버리 상태 수",
    "recovery.result.borrowed": "선행 사용한 2단계 배치 수",
    "recovery.result.checkpoint": "1단계 정리 시점",
    "recovery.result.evaluated": "평가한 / 전체 가능 패턴",
    "recovery.result.normal_probability": "정상 PC 확률",
    "recovery.result.pc_recovery_probability": "PC 보존 리커버리 확률",
    "recovery.result.non_pc_recovery_probability": "PC 없는 리커버리 확률",
    "recovery.result.additional_probability": "추가 리커버리 확률",
    "recovery.result.response_probability": "총 대응 확률",
    "recovery.result.no_path_probability": "경로 없음 확률",
    "recovery.result.unknown_probability": "미확정 확률",
    ...
    "input.options.setup_qb_required": "셋업 mode=qb에는 qb 큐 또는 패턴이 필요합니다.",
    "input.options.setup_qb_oracle_conflict": "셋업 mode=oracle에서는 qb 공급을 사용할 수 없습니다.",
    "input.options.setup_qb_bag_capacity": "셋업 remaining 개수와 qb 시퀀스 길이의 합은 최대 11이어야 합니다.",
