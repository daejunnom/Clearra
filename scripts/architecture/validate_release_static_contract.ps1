...
function Invoke-ReleaseIdentityGateValidation {
    ...
    foreach ($required in @(
        ...
        'gcloud storage cp evidence/cloud-build-inputs.tar.gz "$cloud_input_object"',
        '--if-generation-match=0',
        'gcloud builds submit "$cloud_input_object"',
        '--storage-source-generation "$cloud_input_generation"',
    ...
    if ($captureSectionStart -lt 0 -or $restoreSectionStart -le $captureSectionStart) {
        ...
        if ($modernPrepareStart -lt 0 -or
            ...
            foreach ($acceptedDownloadContract in @(
                'artifact-ids: ${{ needs.capture-authority.outputs.accepted_artifact_id }}',
                'merge-multiple: true',
                'run-id: ${{ needs.capture-authority.outputs.accepted_run_id }}',
                'path: canonical-pages-build'
            )) {
                if (-not $modernPrepareStep.Contains($acceptedDownloadContract)) {
                    Add-ArchitectureError "Pages canonical capture must preserve exact artifact identity and flat extraction root: $acceptedDownloadContract"
                }
            }
    ...
    foreach ($required in @(
        ...
        '756ab15f1bb3554191833abf1cb362a126c9eacfae3b21e884949445df37ede1'
    )) {
