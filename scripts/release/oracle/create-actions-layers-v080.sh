...
usage() {
  printf '%s\n' 'usage: create-actions-layers-v080.sh <repository-root> <accepted-ctk3-dist-directory> <source-commit> <accepted-run-id> <accepted-run-attempt> <new-output-directory>' >&2
  exit 64
}
...
output_root="$6"
[[ "$accepted_ctk3_root" = /* && -d "$accepted_ctk3_root" && ! -L "$accepted_ctk3_root" ]] || usage
...
[[ "$output_root" = /* && -d "$output_root" && ! -L "$output_root" ]] || usage
output_root="$(cd "$output_root" && pwd -P)"

[[ "$accepted_ctk3_root" != "$repository_root/packages/ctk3/dist" ]] || usage
...
for output in "$dist_archive" "$dependencies_archive"; do
  [[ ! -e "$output" && ! -L "$output" ]] || {
    printf 'refusing to overwrite frozen Actions layer: %s\n' "$output" >&2
    exit 73
  }
done

temporary_root="$(mktemp -d /tmp/clearra-v080-actions-layers.XXXXXXXX)"
cleanup() {
  status=$?
  trap - EXIT HUP INT TERM
  case "$temporary_root" in
    /tmp/clearra-v080-actions-layers.*)
...
mkdir -p "$temporary_root/node_modules/@clearra" "$temporary_root/packages/ctk3"
...
publish_archive() {
  destination=$1
  shift
  temporary_archive="$(mktemp "$output_root/.clearra-v080-actions-layer.XXXXXXXX")"
  tar "${tar_flags[@]}" --file="$temporary_archive" "$@"
  chmod 0600 "$temporary_archive"
  if ! ln -- "$temporary_archive" "$destination"; then
    rm -f -- "$temporary_archive"
    printf 'frozen Actions layer appeared during publication: %s\n' "$destination" >&2
    exit 73
  fi
  rm -f -- "$temporary_archive"
}
...
printf 'oracle_actions_ctk3_authority=accepted source_commit=%s run_id=%s run_attempt=%s\n' \
  "$source_commit" "$accepted_run_id" "$accepted_run_attempt"
for output in "$dist_archive" "$dependencies_archive"; do
  [[ -f "$output" && ! -L "$output" ]] || exit 73
  printf 'oracle_actions_layer=%s sha256=%s size=%s\n' \
    "$(basename "$output")" \
