...
test("v0.8 Oracle inactive-stage generator binds every frozen authority", () => {
  ...
  const candidateValidationCalls = text.split("\n").filter((line) =>
    /^\s*validate_candidate\s/u.test(line));
  assert.equal(candidateValidationCalls.length, 5);
  for (const call of candidateValidationCalls) {
    assert.match(call, /^\s*validate_candidate "\$[a-z_]+" "\$(?:cleanup_digester|input_root\/clearra-release-tree-digest\.py)"$/u);
  }
  assert.match(
    text,
    /validate_candidate "\$assemble_root" "\$input_root\/clearra-release-tree-digest\.py"/u,
  );
