...
test("prestage checks Cloud permission after prior authority and before the first Oracle call", () => {
  ...
  const preflight = prestage.indexOf("Assert-PrestageCloudCleanupPermission");
...
test("tagless prior state needs no permission-changing request and emits no authority", psOptions, () => {
  runPs(`
$script:Service.status.traffic = @($script:Service.status.traffic[0])
$result = @(Assert-PrestageCloudCleanupPermission -Service $script:Service -Intent $intent -PriorRevision $prior.prior_revision)
if ($script:Calls -ne 0 -or $result.Count -ne 0) { throw 'tagless preflight must not invoke the cleanup helper' }
`);
});

test("tagged prior state uses only validate-only and exact original intent arguments", psOptions, () => {
  const result = runPs(`
$output = @(Assert-PrestageCloudCleanupPermission -Service $script:Service -Intent $intent -PriorRevision $prior.prior_revision)
...
test("actAs denial remains the top-level cause instead of a generic validator failure", psOptions, () => {
  runPs(`
$script:ChildExit = 77
try {
...
test("foreign prior and ambiguous candidate tags fail before helper invocation", psOptions, () => {
  runPs(`
try { Assert-PrestageCloudCleanupPermission -Service $script:Service -Intent $intent -PriorRevision 'foreign'; throw 'unexpected success' }
...
try { Assert-PrestageCloudCleanupPermission -Service $script:Service -Intent $intent -PriorRevision $prior.prior_revision; throw 'unexpected success' }
...
test("actual prestage branch stops at denied preflight without an Oracle artifact or result", psOptions, async () => {
...
if ($files.Count -ne 1 -or $files[0].Name -notlike 'cloud-prestage-before-*.json') { throw 'work continued after permission denial' }
