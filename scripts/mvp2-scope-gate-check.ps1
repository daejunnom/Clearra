...
try {
    Invoke-CheckedCommand `
        -Label "cargo check -p clearra-validation" `
        -FileName $CargoPath `
        -Arguments @("check", "-p", "clearra-validation")

    Invoke-CheckedCommand `
        -Label "architecture X0 MVP2 Scope Gate" `
        -FileName "powershell" `
        -Arguments @(
            "-NoProfile",
            "-File", "scripts/validate_architecture.ps1",
            "-TaskName", "X0 MVP2 Scope Gate"
        )

    Complete-ClearraProgressLine $script:Scope
    Write-Output "[mvp2-scope] passed | execution_surface=compile-and-architecture-only | test_executable_launched=false"
}
finally {
    Pop-Location
}
