...
try {
    ...
    Invoke-CheckedCommand `
        -Label "architecture G1 Custom Piece Domain Model" `
        -FileName "powershell" `
        -Arguments @(
            "-NoProfile",
            "-File", "scripts/validate_architecture.ps1",
            "-TaskName", "G1 Custom Piece Domain Model"
        )

    Complete-ClearraProgressLine $script:Scope
    Write-Output "[custom-piece] passed | execution_surface=compile-and-architecture-only | test_executable_launched=false"
}
finally {
    Pop-Location
}
