...
try {
    ...
    Invoke-CheckedCommand `
        -Label "architecture G5 Area Multiset Feasibility" `
        -FileName "powershell" `
        -Arguments @(
            "-NoProfile",
            "-File", "scripts/validate_architecture.ps1",
            "-TaskName", "G5 Area Multiset Feasibility"
        )

    Complete-ClearraProgressLine $script:Scope
    Write-Output "[area-feasibility] passed | execution_surface=compile-and-architecture-only | test_executable_launched=false"
}
finally {
    Pop-Location
}
