...
try {
    ...
    Invoke-CheckedCommand `
        -Label "architecture G0 MVP3 Scope Gate" `
        -FileName "powershell" `
        -Arguments @(
            "-NoProfile",
            "-File", "scripts/validate_architecture.ps1",
            "-TaskName", "G0 MVP3 Scope Gate"
        )

    Complete-ClearraProgressLine $script:Scope
    Write-Output "[mvp3-scope] passed | execution_surface=compile-and-architecture-only | test_executable_launched=false"
}
finally {
    Pop-Location
}
