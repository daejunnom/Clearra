...
try {
    ...
    Invoke-CheckedCommand `
        -Label "architecture G4 Generic Operation Candidate Reachability" `
        -FileName "powershell" `
        -Arguments @(
            "-NoProfile",
            "-File", "scripts/validate_architecture.ps1",
            "-TaskName", "G4 Generic Operation Candidate Reachability"
        )

    Complete-ClearraProgressLine $script:Scope
    Write-Output "[generic-operation] passed | execution_surface=compile-and-architecture-only | test_executable_launched=false"
}
finally {
    Pop-Location
}
