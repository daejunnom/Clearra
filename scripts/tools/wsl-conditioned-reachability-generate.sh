...
hash -r
case "$PATH" in *"/mnt/"*) printf 'Windows PATH entry leaked into WSL conditioned-reachability generation\n' >&2; exit 2 ;; esac
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *) printf 'Unsupported conditioned-reachability generation argument: %s\n' "$1" >&2; exit 2 ;;
    esac
done

case "$PROFILE" in srs | srs-plus | srs-x | jstris-180 | no-kick) ;; *)
    printf 'Unsupported conditioned-reachability kick profile: %s\n' "$PROFILE" >&2
    exit 2
    ;;
esac
[[ "$QUERIES" == /mnt/?/* && -f "$QUERIES" && ! -L "$QUERIES" ]] || {
    printf 'Conditioned-reachability queries must be a validated host file\n' >&2
    exit 2
}
for output in "$PACK" "$CATALOG"; do
    [[ "$output" == /mnt/?/* && -d "$(dirname "$output")" && ! -L "$(dirname "$output")" && ! -L "$output" ]] || {
        printf 'Conditioned-reachability output must have a validated host parent\n' >&2
        exit 2
    }
done
[[ "$PACK" != "$CATALOG" && "$PACK" != "$QUERIES" && "$CATALOG" != "$QUERIES" ]] || {
    printf 'Conditioned-reachability query and output paths must be distinct\n' >&2
    exit 2
}
[[ "$WORKERS" =~ ^[1-9][0-9]*$ && "$WORKERS" -le "$(nproc)" ]] || {
    printf 'Conditioned-reachability workers must be within 1..nproc; no silent reduction is permitted\n' >&2
    exit 2
}
...
if [[ -z "${CLEARRA_BUILD_SESSION_ID:-}" ]]; then
    exec node "$AUTHORITY_ROOT/scripts/tools/invoke-clearra-build.mjs" \
        ...
        -- bash "$AUTHORITY_ROOT/scripts/tools/wsl-conditioned-reachability-generate.sh" \
        --profile "$PROFILE" --queries "$QUERIES" --pack "$PACK" \
        --catalog "$CATALOG" --workers "$WORKERS"
fi
...
mkdir -p "$BUILD_TRANSACTION/conditioned-reachability-generator"

bash "$AUTHORITY_ROOT/scripts/tools/wsl-native-cargo.sh" \
    build --locked --release --no-default-features \
    --features legal-board-assets \
    --package clearra-pc4-qualifier --bin clearra-conditioned-reachability

BINARY="$MANAGED_CARGO_TARGET/release/clearra-conditioned-reachability"
[[ -x "$BINARY" && ! -L "$BINARY" ]] || {
    printf 'Managed conditioned-reachability generator is missing: %s\n' "$BINARY" >&2
    exit 2
}
exec "$BINARY" --profile "$PROFILE" --queries "$QUERIES" \
    --pack "$PACK" --catalog "$CATALOG" --workers "$WORKERS"
