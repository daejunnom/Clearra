...
const en = {
  ...
  setupResidueHelp: 'Enter every piece remaining before the bag boundary. They form an unordered residue pattern, so input order never fixes placement order. Include an existing hold piece; one duplicated kind identifies that hold.',
  pcCycle: 'PC cycle',
  ...
  setupPriorityAll: 'All',
  setupPriorityBuild: 'Build probability first',
  setupPriorityPc: 'PC probability first',
  setupPriorityHelp: 'Build-first breaks equal build probabilities by PC probability. PC-first breaks equal PC probabilities by build probability.',
  allowPostCycleBorrow: 'Allow one piece after cycle reset',
  postCycleBorrowHelp: 'Cycle 7 only. Allows one setup lock from the newly reset bag after completing the current PC cycle.',
  ...
  lockCount: '{count} locks',
  lockRange: '{min}-{max} locks',
...
const ko: Record<keyof typeof en, string> = {
  ...
  setupResidueHelp: '가방 경계 전까지 남은 미노를 모두 입력합니다. 입력은 순서 없는 잔여 패턴이므로 작성 순서가 배치 순서를 고정하지 않습니다. 기존 홀드는 함께 넣으며, 정확히 한 종류의 중복이 그 홀드를 나타냅니다.',
  pcCycle: 'PC 회차',
  ...
  setupPriorityAll: '전체',
  setupPriorityBuild: '구축 가능성 우선',
  setupPriorityPc: 'PC 확률 우선',
  setupPriorityHelp: '구축 가능성이 같으면 PC 확률을, PC 확률이 같으면 구축 가능성을 비교합니다.',
