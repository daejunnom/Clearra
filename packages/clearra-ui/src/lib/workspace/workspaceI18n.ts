...
const en = {
  ...
  setupModeQbHelp: 'Find current-cycle setups whose completed PC leaves exactly the requested inventory for the next PC cycle.',
  remainingPieces: 'Remaining pieces',
  setupNextCycleRemaining: 'Next-cycle remaining pieces',
  setupQbInputHelp: 'Enter the exact hold plus bag remainder wanted after this PC. One piece kind may occur twice when one copy is held.',
  ...
  setupQbQueueLetters: 'The first group is this cycle residue; the second is the exact inventory requested at the next PC-cycle boundary.',
  setupQbAllPieces: 'The QB inventory is a terminal supply constraint, not a fixed queue prefix. Clearra derives every compatible current queue without forcing those pieces into the setup.',
  setupQbSevenLimit: 'The required count follows the next cycle automatically. A held piece may duplicate one piece from the remaining standard bag.',
  ...
  setup_qb_count_invalid: 'The next-cycle inventory must contain exactly the piece count required by the cycle reached after this PC.',
  setup_qb_piece_invalid: 'Next-cycle inventory may contain only I, O, T, S, Z, J, and L.',
  setup_qb_duplicate_invalid: 'Only one piece kind may repeat, and it may occur at most twice through hold carryover.',
...
const ko: Record<keyof typeof en, string> = {
  ...
  setupModeQbHelp: '현재 PC를 마친 뒤 요청한 다음 회차 잔여 미노를 정확히 남기는 셋업을 찾습니다.',
  remainingPieces: '남은 미노',
  setupNextCycleRemaining: '다음 회차 남은 미노',
  setupQbInputHelp: '이번 PC 뒤에 남기려는 hold와 bag 나머지를 정확히 입력합니다. hold의 한 미노는 bag 나머지와 한 종류까지 중복될 수 있습니다.',
  ...
  setupQbQueueLetters: '첫 그룹은 현재 회차 잔여이며, 둘째 그룹은 PC 뒤 다음 회차 경계에서 정확히 남길 공급입니다.',
  setupQbAllPieces: 'QB 입력은 고정 큐 접두부가 아니라 terminal 공급 조건입니다. 해당 미노를 셋업에 강제로 쓰지 않고 양립 가능한 현재 큐를 역으로 계산합니다.',
  setupQbSevenLimit: '필요 개수는 다음 회차에서 자동 결정됩니다. hold carryover 때문에 한 종류는 bag 나머지와 중복될 수 있습니다.',
  ...
  setup_qb_count_invalid: '다음 회차 잔여는 이번 PC 뒤 도달하는 회차에 필요한 개수와 정확히 같아야 합니다.',
  setup_qb_piece_invalid: '다음 회차 잔여에는 I, O, T, S, Z, J, L만 사용할 수 있습니다.',
  setup_qb_duplicate_invalid: 'hold carryover로 한 종류만 중복할 수 있으며 같은 미노는 최대 2개까지 허용됩니다.',
